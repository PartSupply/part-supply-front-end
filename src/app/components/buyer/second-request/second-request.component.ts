...
@Component({
  selector: 'app-second-request',
  templateUrl: './second-request.component.html',
  styleUrls: ['./second-request.component.css']
})
export class SecondRequestComponent implements OnInit {
  name = "jinal";
  vehicleInfoForm2 = createNewFormControl();
  ...
  constructor(private httpService: HttpService, public dialog: MatDialog, @Optional() @Inject(MAT_DIALOG_DATA) public data: any) { 
    ...
    this.partData = data;
    console.log('hey i m part data', this.partData);
  }
  public async postPartRequest(): Promise<void> {
    const payload = {
      year: this.partData.year,
      make: this.partData.make,
      model: this.partData.model,
      vinNumber: this.partData.vinNumber,
      partName: this.vehicleInfoForm2.get('partName').value,
      partType: {
        new: this.vehicleInfoForm2.get('partType.new').value,
        used: this.vehicleInfoForm2.get('partType.used').value,
        reManufactured: this.vehicleInfoForm2.get('partType.reManufactured').value,
      },
      numberOfOffers: 0,
      offerStatus: 'OPEN'
    }
